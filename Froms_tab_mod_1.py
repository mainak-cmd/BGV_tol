...
def All_docs_from():
    layout = [
        ...
        [sg.Stretch(),sg.Button('Right_to_work'),sg.Checkbox('Right_to_work', key='Right_to_work'), sg.Stretch()],
    ...
    while True:
        ...
        if values['Right_to_work6'] == True and event == "Right_to_work":
            Back_ground_from.Right_to_work()
        if event == "Submit":
            for i in values:
                while True:
                    if not values[i]:
